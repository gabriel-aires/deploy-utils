...
if [ -z "$QUERY_STRING" ]; then
    ...
    head -n 1 $history_dir/$history_csv_file > $tmp_dir/header 2> /dev/null

    # Exibir texto explicativo
    echo "<div class=\"column zero_padding box_shadow cfg_color\">"
    echo "  <table>"
    echo "      <tr><th colspan=\"3\">UTILIZAÇÃO:</th></tr>"
    echo "      <tr><th>INSTRUÇÃO</th><th>DESCRIÇÃO</th><th>EXEMPLO</th></tr>"
    echo "      <tr><td>SELECT:</td><td>Especificar a colunas a serem selecionadas.</td><td> Ex: $(test -f $history_dir/$history_csv_file && query_file.sh -d "$delim" -r ' ' -s all -t 1 -f $tmp_dir/header) (padrão=all)</td></tr>"
