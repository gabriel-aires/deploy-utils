...
if $change_color; then
    sed -i -r "s|^(.*)<th>$col_flag_name</th><th>$|\t\t\t<tr class=\"header_color\"><th>\1</tr>|" $tmp_dir/html
    sed -i -r "s|^(.*)<td>1</td><td>$|\t\t\t<tr class=\"query_row_default\"><td>\1</tr>|" $tmp_dir/html
    sed -i -r "s|^(.*)<td>0</td><td>$|\t\t\t<tr class=\"query_row_warning\"><td>\1</tr>|" $tmp_dir/html
else
    sed -i -r "s|^(.*)<th>$|\t\t\t<tr class=\"header_color\"><th>\1</tr>|" $tmp_dir/html
    sed -i -r "s|^(.*)<td>$|\t\t\t<tr class=\"query_row_default\"><td>\1</tr>|" $tmp_dir/html
fi
